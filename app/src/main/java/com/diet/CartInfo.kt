...
class CartInfo() : AppCompatActivity() {
    ...
    //var accountList = arrayListOf<OrderDTO>()
    ...
    private fun getCartInfoByUserID() {
        val product = ProductDTO()

        product.userId = Utils.getUserId(applicationContext)

        val res: Call<JsonObject> =
            ProductApiRetrofit.getInstance(applicationContext).service.getCartInfoByUserID(product)
        res.enqueue(object : Callback<JsonObject?> {
            override fun onResponse(call: Call<JsonObject?>, response: Response<JsonObject?>) {
                ...
                if (response.isSuccessful) {
                    ...
                    ////
                    recyclerView = findViewById(R.id.recyclerCartProduct)
                    var CartProductAdapter = CartProductAdapter(applicationContext, accountList)                   // CustomaApdater() 를 저장하는 apdater 변수 생성
                    val recyclerDecoration = RecyclerViewDecoration(20,5)

                    recyclerView.addItemDecoration(recyclerDecoration)
                    recyclerView.adapter = CartProductAdapter // CustomAdapter에 선언한 ProductList 변수에 data  변수 전달
